...
| Project | duration | Overview | Stack | Link |
|------|------|------|------|------|
| ☕ AI Kiosk for Cafe (Korean) | `2025.03.17 ~ 2025.04.06 `| AI 키오스크 프로젝트는 카페 환경에서 음성 기반 자연어 인터페이스를 통해 고객이 주문을 보다 직관적으로 진행할 수 있도록 돕는 **음성 주문 챗봇 시스템**입니다.고객은 대화하듯 메뉴를 문의하거나 영양 정보를 확인하고, 원하는 메뉴를 장바구니에 담아 최종 주문까지 자연스럽게 이어갈 수 있습니다. | `Langchain` `LLM` `FastAPI` `SQL`  |🔗[Repo](https://github.com/Wanted-AI-project/Korean_AI_Kiosk_Agent)
| 🎮 AI를 이용한 게임 내 음성명령 시스템 | `2025.03.13 ~ 2025.03.14 `|게임 내 몬스터에게 음성 명령을 전달하면, 그 명령을 수행하게 하는 **음성 명령 변환 시스템**입니다. 사용자의 음성 명령을 받아 자연어 텍스트로 전환 -> 텍스트를 JSON 형태로 파싱하도록 LLM모델을 튜닝 -> RestAPI 서버로 전달. | `Langchain` `FastAPI` |🔗[Repo](https://github.com/0-9-10-12/VoiceCommand)
| 🎤 최애가 읽어주는 뉴스 | `2025.02.20~2025.03.05 `|AI 캐릭터 음성을 통해, 내가 원하는 주제의 뉴스를 요약해서 들려주는 맞춤형 뉴스 리더 서비스입니다. | `LLM` `vLLM` `Linux` `HuggingFace` `FastAPI`|🔗[Repo](https://github.com/Wanted-Potenup/News_Reader_CharacterVoice)
